...
async function run() {
  try {
    ...
    if (bundleId) {
      ...
      if (rawProfileIds) {
        const profilesResponse = await get("https://api.appstoreconnect.apple.com/v1/profiles", { "filter[id]": `9X8F3VXG4W`, "filter[profileType]": signType }, token); // ProfilesResponse Type
