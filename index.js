...
async function run() {
  try {
    ...
    if (bundleId) {
      ...
      if (rawProfileIds) {
        const profilesResponse = await get("https://api.appstoreconnect.apple.com/v1/profiles", { "filter[id]": `${rawProfileIds[0]}`, "filter[profileType]": signType }, token); // ProfilesResponse Type
